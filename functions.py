...
def detect_knee_bounding_boxes2(dicom_data):
    ...
    _, binary_image = cv2.threshold(cv2.cvtColor(blurred_image, cv2.COLOR_BGR2GRAY), 140, 255, cv2.THRESH_BINARY)
    ...
    sobel_edge_resized = cv2.resize(sobel_edge.astype(np.uint8), (multichannel_image.shape[1], multichannel_image.shape[0]))
    ...
    return bounding_boxes, multichannel_image

# Define the function to calculate the area of a bounding box
def calculate_area(bbox):
    _, _, w, h = bbox
    return w * h

# Define the function to process DICOM files and identify top 5 bounding boxes with the maximum area decrease
def process_dicom_files2(dicom_folder):
    dicom_files = read_dicom_folder(dicom_folder)
    
    # Define the range of slices to process (4/10 to 6/10)
    start_index = int(len(dicom_files) * 0.4)
    end_index = int(len(dicom_files) * 0.6)
    
    previous_bounding_boxes = []
    bounding_box_differences = []

    # Identify the top bounding box A with the maximum area decrease
    top_bounding_box = None
    for dicom_file in dicom_files[start_index:end_index]:
        dicom_data = pydicom.dcmread(dicom_file)
        bounding_boxes, multichannel_image = detect_knee_bounding_boxes2(dicom_data)
        
        # If there were bounding boxes in the previous slice, compare them
        if previous_bounding_boxes:
            for current_box in bounding_boxes:
                current_area = calculate_area(current_box)
                for previous_box in previous_bounding_boxes:
                    previous_area = calculate_area(previous_box)
                    area_difference = previous_area - current_area
                    if area_difference > 0:
                        bounding_box_differences.append((area_difference, current_box, multichannel_image, dicom_file))
        
        # Update the previous bounding boxes
        previous_bounding_boxes = bounding_boxes

    # Sort bounding boxes by area difference
    bounding_box_differences.sort(key=lambda x: x[0], reverse=True)

    # Get the top 5 bounding boxes with the maximum decrease in area
    top_5_differences = bounding_box_differences[:5]

    # Find the largest bounding box among the top 5
    largest_bounding_box = max(top_5_differences, key=lambda x: calculate_area(x[1]))

    # Find the 20th slice comparison from A and determine the target slice
    index_of_A = dicom_files.index(largest_bounding_box[3])
    comparison_index = index_of_A + 20
    
    if comparison_index < len(dicom_files):
        dicom_data_comparison = pydicom.dcmread(dicom_files[comparison_index])
        bounding_boxes_comparison, _ = detect_knee_bounding_boxes2(dicom_data_comparison)

        # Compare bounding boxes with A's bounding boxes
        if any(calculate_area(bbox_comparison) < calculate_area(largest_bounding_box[1]) for bbox_comparison in bounding_boxes_comparison):
            target_slice = 16
        ...
        # Display the target slice only 
        if target_slice == 16 or target_slice == 40:
            dicom_data_new = pydicom.dcmread(dicom_files[index_of_A + target_slice])
            bounding_boxes_new, _ = detect_knee_bounding_boxes2(dicom_data_comparison)
